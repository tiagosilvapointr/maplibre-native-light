...
void LayerManagerDarwin::registerCoreFactory(LayerFactory* factory) {
  ...
  if (!type.empty()) {
    NSCAssert(typeToFactory.find(type) == typeToFactory.end(),
              @"A layer type can be registered only once.");
...
// static
LayerManager* LayerManager::get() noexcept { return LayerManagerDarwin::get(); }
