...
- (void)commonInit {
  ...
  self.accessibilityLabel = NSLocalizedStringWithDefaultValue(@"COMPASS_A11Y_LABEL", nil, nil,
                                                              @"Compass", @"Accessibility label");
  self.accessibilityHint = NSLocalizedStringWithDefaultValue(
...
- (void)setCompassVisibility:(MLNOrnamentVisibility)compassVisibility {
  if (_compassVisibility == compassVisibility) {
    return;
  }
...
- (UIImage *)compassImage {
  ...
  NSAttributedString *north = [[NSAttributedString alloc]
      initWithString:NSLocalizedStringWithDefaultValue(@"COMPASS_NORTH", nil, nil, @"N",
...
- (void)animateToAlpha:(CGFloat)alpha {
  [UIView animateWithDuration:MLNAnimationDuration
                        delay:0
